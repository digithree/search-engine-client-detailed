	...
	if (result.count > 0) {
		...
		if (searchResultExists) {
			result.links = await nm.evaluate((selector) => {
					let links = elements.map(elements => elements.href)
            ...
            result.emails=await nm.evaluate((selector) => {
                let re = /[^\s@]+@[^\s@]+\.[^\s@]+/g
                  , elements=document.body.innerText
                  , found
                  , emails=[];

                do {
                    found = re.exec(elements);
                    if(Array.isArray(found)){
                        emails=emails.concat(found.map(i=>i));
                    }
                } while (found);
			...
			while (true) {
                ...
                let next_emails=await nm.evaluate((selector) => {
                    let re = /[^\s@]+@[^\s@]+\.[^\s@]+/g
                      , elements=document.body.innerText
                      , found
                      , emails=[];
				...
				result.emails = unique(result.emails.concat(next_emails));
			}
		} else {
			result.links = [];
			result.error = true;
