...
const SEARCH_ENGINE_URL = "https://searchdns.netcraft.com/";
const COUNT_SELECTOR = ".blogbody p em";
const LINK_SELECTOR = ".TBtable tbody tr";

const NEXT_SELECTOR = ".blogbody p a";
const ENGINE = Path.basename(__filename, ".js");
const PERSIST_NAME = "sec-" + ENGINE;
const SEARCH_TEXTBOX_SELECTOR = 'form[action="/"] [name="host"]';
const SEARCH_SUBMIT_SELECTOR = 'form[action="/"] [type="submit"]';
const CHECK_SELECTOR = 'form[action="/"] select[name="restriction"]';
	...
	if (result.count > 0) {
		...
		if (searchResultExists) {
			result.links = await nm.evaluate((selector) => {
                    ...
                    var links = elements.splice(1).map(element =>{
                        var data=Array.from(element.children).map(c => c.innerText);
